...
const FISA_SLUG = "fisa-vizionare-inchiriere";
const docType = getDocumentTypeBySlug(FISA_SLUG)!;
const STORAGE_KEY_MODE = "fisa-vizionare-inchiriere-mode";
...
export function FisaVizionareInchiriereFlow() {
  ...
  const handleCreateLink = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/documents/fisa-vizionare-inchiriere/remote/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: user?.id ?? "anonymous",
          ...agentForm,
          agency_denumire: settings.agency.denumire ?? "",
          agency_sediu: settings.agency.sediu_social ?? "",
          agency_cui: settings.agency.cui ?? "",
          comision_termen_plata_zile: "7",
  ...
  const handlePreviewPdf = async () => {
    ...
    try {
      const settings = getAgencyAgentSettings();
      const res = await fetch("/api/documents/fisa-vizionare-inchiriere/remote/preview-pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...agentForm,
  ...
  const whatsappUrl = linkResult?.link
    ? `https://wa.me/?text=${encodeURIComponent("Te invit să semnezi fișa de vizionare (închiriere): " + linkResult.link)}`
    : null;
        ...
        <>
          <div className="flex justify-end">
            <Button variant="outline" size="sm" asChild>
              <Link href="/documents/signed/inchiriere">
                <FileCheck className="mr-2 h-4 w-4" />
                Documente semnate (Închiriere)
              </Link>
            </Button>
          </div>
          <Card className="border-purple-100 bg-white">
            <CardHeader>
              <h2 className="text-lg font-semibold">DETALII VIZIONARE ÎNCHIRIERE (COMPLETATE DE AGENT)</h2>
              <p className="text-sm text-muted-foreground">
                Completează detaliile; linkul generat îl trimiți clientului pentru semnare.
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Termenul de plată a comisionului este fix: 7 zile calendaristice de la finalizarea tranzacției.
              </p>
              <p className="text-sm text-muted-foreground">
                Datele agenției (Denumire, Sediu, CUI) și ale agentului se completează din Setări.
              </p>
