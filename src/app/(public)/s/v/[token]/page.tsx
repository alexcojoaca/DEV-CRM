...
import { FisaVizionareFullText } from "@/components/documents/FisaVizionareFullText";
...
export default function SignViewingPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/documents/fisa-vizionare-vanzare/remote/${token}`)
      .then((r) => {
  ...
  const handleSign = async () => {
    ...
    setSigning(true);
    try {
      const res = await fetch(`/api/documents/fisa-vizionare-vanzare/remote/${token}/sign`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          // Semnătură digitală: fără imagine; serverul înregistrează IP + data/ora
        }),
      });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900">Fișă de vizionare — Vânzare</h1>
          <p className="text-sm text-gray-600 mt-1">
            Citește integral documentul mai jos, apoi completează datele și apasă „Semnează” pentru semnătură electronică (se înregistrează data, ora și adresa IP).
          </p>
        </div>
        ...
        <div className="flex justify-center">
          <Button
            ...
            onClick={() => window.open(`/api/documents/fisa-vizionare-vanzare/remote/${token}/preview-pdf`, "_blank", "noopener,noreferrer")}
          >
        ...
        <div>
          <p className="text-xs font-medium text-gray-600 uppercase tracking-wide mb-2">Documentul pe care îl semnezi (citește tot)</p>
          <FisaVizionareFullText
            dataVizionarii={info.data_vizionarii}
            oraVizionarii={info.ora_vizionarii}
            tipImobil={info.tip_imobil}
            adresaPublica={info.adresa_publica}
        ...
        <Card className="border border-gray-300 bg-white shadow-md">
          <CardHeader>
            ...
            <p className="text-xs text-gray-600">
              După ce ai citit documentul, completează și apasă „Semnează”. Semnătura este electronică (se înregistrează automat data, ora și adresa IP).
            </p>
          </CardHeader>
