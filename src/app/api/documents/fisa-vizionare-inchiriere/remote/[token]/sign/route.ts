...
import { getSession, markSigned } from "@/lib/signing-sessions-inchiriere";
...
const PDF_FILENAME = "fisa-vizionare-inchiriere-semnata.pdf";
...
export async function POST(
  ...
  try {
    ...
    if (session.status === "signed") {
      return NextResponse.json({ error: "Documentul a fost deja semnat." }, { status: 409 });
    }
    ...
    const ciSerie = (body.ci_serie ?? "").toString().trim();
    const ciNumar = (body.ci_numar ?? "").toString().trim();
    const ciStr = ciSerie && ciNumar ? ciSerie + " " + ciNumar : ciSerie || ciNumar || "";
    ...
    const payload = {
      ...
      ci_serie_numar: ciStr,
      ci_serie: ciSerie || undefined,
      ci_numar: ciNumar || undefined,
    ...
    console.error("Remote sign inchiriere error:", e);
    return NextResponse.json({ error: "Eroare la semnare." }, { status: 500 });
  }
}
